  ...
  const startRender = () => {
    ...
    const noiseArray = (x, y, noise) => {
      ...
      for (let i = 0; i < x; i++) {
        arr[i] = []
        for (let j = 0; j < y; j++) {
          arr[i][j] = noise(i * frequency, j * frequency) * amplitude
        }
      }
      return arr
    }
  ...
  return (
    <div className='w-screen h-screen m-0 p-0 relative bg-gray-400'>
      ...
      <div className={`absolute top-[20vh] right-0 w-[20vw] h-[60vh] flex flex-col bg-gray-700 borderRadius p-8 opacity-90 z-60 ${menuFold ? "" : "foldAway"}`}>
        <div className='flex'>
          <p className='displayFont text-2xl text-white w-1/2'>Terrain generator!</p>
        ...
        <p className='text-white text-xl displayFont mt-16'>Amplitude:</p>
